...
def compute_component_q(resps, camera_locations, 
    ...
    return np.where(normalizer!=0, np.sum(resps * nll_vec * obs_mask)/normalizer, 0)
...
def compute_model_nll(camera_locations, 
    ...
    nll = -np.sum(np.log(per_example_ll+eps)*obs_mask)/np.sum(obs_mask)
...
def em_step(camera_locations, 
    ...
    if not parallel: 
        ...
        in_axes = (1, None, None,  None, None, None, None, 0, None, None, None)
